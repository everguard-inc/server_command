...
Then deploy from the proxy:
  ...
  python3 deploy_clients.py --hosts 10.230.0.100
  python3 deploy_clients.py --dry-run

--deps runs setup_client.py remotely (pip, systemd, sudoers, service restart).
...
from edge_sudoers import REMOTE_SHELL, systemctl_paths
...
def setup_client_env_steps(sudo_password, username):
  ...
  return [
    ...
    "python3 setup_client.py --skip-git",
  ]


def setup_client_remote_script(repo_path, sudo_password, username):
  """Run setup_client.py on a remote host with SUDOPASS/SUDO_USER set reliably."""
  repo = shlex.quote(repo_path)
  return shell_script(f"cd {repo}", *setup_client_env_steps(sudo_password, username))


def git_pull_script(repo_path, *, with_setup, sudo_password, username):
  repo = shlex.quote(repo_path)
  steps = [f"cd {repo}", "git pull"]
  if with_setup:
    steps.extend(setup_client_env_steps(sudo_password, username))
  else:
...
def deploy_host(host, args, runner):
  if args.local:
    ...
    script = setup_client_remote_script(args.repo_path, args.sudo_password, args.user)
...
def build_parser():
  ...
  p.add_argument("--deps", action="store_true",
                 help="after sync/pull: run setup_client.py (pip, systemd, sudoers, restart)")
